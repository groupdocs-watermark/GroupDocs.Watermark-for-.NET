...
using GroupDocsWatermarkVisualStudioPlugin.GUI;
using GroupDocsWatermarkVisualStudioPlugin.Properties;

namespace GroupDocsWatermarkVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Watermark for .NET API Examples";

        private const string WATERMARK_COMMAND_NAME = "GroupDocsWatermarkVSPlugin";
        private const string WATERMARK_COMMAND_CAPTION = "New GroupDocs.Watermark Example Project";
        private const string WATERMARK_COMMAND_TOOLTIP = "Download and Run GroupDocs.Watermark for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == WATERMARK_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, WATERMARK_COMMAND_NAME,
                        WATERMARK_COMMAND_CAPTION, WATERMARK_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + WATERMARK_COMMAND_NAME)
                    {
