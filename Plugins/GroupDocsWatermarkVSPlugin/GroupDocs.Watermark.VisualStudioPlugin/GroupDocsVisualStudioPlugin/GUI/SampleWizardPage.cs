...
using GroupDocsWatermarkVisualStudioPlugin.Core;
using System.IO;
using System.Threading;
using System.Xml;
using GroupDocsWatermarkVisualStudioPlugin.XML;
using System.Xml.Serialization;
using System.Diagnostics;
using System.Xml.Linq;
using EnvDTE80;

namespace GroupDocsWatermarkVisualStudioPlugin.GUI
{
    public partial class SampleWizardPage : Form
    {
        ...
        public SampleWizardPage(DTE2 application)
        {
            ...
            PopulateTreeView(repoPath + "/Examples/" + (rdbCSharp.Checked ? "GroupDocs.Watermark.Examples.CSharp" : "GroupDocs.Watermark.Examples.VBasic"));
        ...
        bool CopyAndCreateProject()
        {
            ...
            //string sampleSourcePath = "GroupDocs.Watermark";
            ...
            try
            {
                ...
                {
                    //For CSharp Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "GroupDocs.Watermark.Examples.CSharp", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "GroupDocs.Watermark.Examples.CSharp", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "GroupDocs.Watermark.Examples.CSharp", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);

                    //For VBasic Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "GroupDocs.Watermark.Examples.VBasic", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "GroupDocs.Watermark.Examples.VBasic", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "GroupDocs.Watermark.Examples.VBasic", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);
                }

                progressBar1.Value = 50;

                string[] projectFiles = Directory.GetFiles(Path.Combine(destinationPath, (rdbCSharp.Checked ? "GroupDocs.Watermark.Examples.CSharp" : "GroupDocs.Watermark.Examples.VBasic")), (rdbCSharp.Checked ? "*.csproj" : "*.vbproj"));
